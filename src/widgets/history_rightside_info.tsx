  ...
  return (
    <>
      {currentCardInfo?.remId ?
        <div className="cursor-pointer grid gap-2 grid-cols-2 history-rightside-widget">
          <div className="flex text-center text-lg items-center gap-2">
            {currentCardInfo?.repetitionHistory?.map((item, index) => {
              return (
                <div key={index} className="flex score-item items-center">
                  ...
                  <div className={'score-hover-infos-overview-right-side'}>
                    {SharedService.translateScore(item.score)} (in {SharedService.convertMilliSecondsIntoLegibleString(item.responseTime)})
                  </div>
                </div>
              );
            })}
